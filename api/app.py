...
@app.route('/companies/<int:company_id>', methods=['GET']) # mapeo automatico de enteros con etiqueta company_id
...
def get_company_by_name(company_name):
    return jsonify(get_company_by_name(company_name)) # las funciones deben tener diferentes nombres
...
@app.route('/companies/<int:company_id>', methods=['PUT']) # methods=['PUT', 'PATCH']
def update_companies(company_id):
    data = request.get_json()
    name = data['name']
    description = data['description']
